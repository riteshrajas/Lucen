...
export default function DashboardLayout({
  ...
  const getActiveTab = () => {
    if (location.pathname.includes('/block-apps')) return 'block-apps';
      ...
      <header className="flex h-14 items-center px-4 lg:px-6 border-b">
          ...
          <SheetContent side="left">
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Link to="/block-apps" className="flex items-center gap-2 text-muted-foreground hover:text-foreground">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <Link to="/block-apps" className={`text-sm ${location.pathname.includes('/block-apps') ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              Block Apps
            </Link>
          </nav>
        </div>
      ...
      <div className="md:hidden flex border-b">
        <Tabs value={getActiveTab()} className="w-full">
          <TabsList className="grid grid-cols-2 w-full">
            ...
            <TabsTrigger value="block-apps" asChild>
              <Link to="/block-apps" className="flex flex-col items-center py-2">
                <Users className="h-4 w-4 mb-1" />
                <span className="text-xs">Block Apps</span>
              </Link>
            </TabsTrigger>
          </TabsList>
        </Tabs>
